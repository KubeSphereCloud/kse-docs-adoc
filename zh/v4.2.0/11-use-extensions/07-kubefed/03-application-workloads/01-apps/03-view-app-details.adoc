...
. 在应用详情页面点击**外部访问**查看应用的外部访问模式。

. 在应用详情页面点击**流量管理**查看应用的流量信息。
+
--
// Note
...
* 如需使用**流量监控**功能，KubeSphere 平台需要安装并启用 KubeSphere 服务网格扩展组件，同时您需要在创建应用时启用应用治理功能。有关更多信息，请参阅link:../01-compose-an-app/[创建自制应用]。
...
* 如需使用**链路追踪**功能，KubeSphere 平台需要安装并启用 KubeSphere 服务网格扩展组件，同时您需要进行以下操作：

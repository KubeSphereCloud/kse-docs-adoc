...
. Execute the following command to download the configuration files **cluster-configuration.yaml** and **kubesphere-installer.yaml**:
+
--
// Bash
...
If your cluster nodes cannot connect to the internet, you can manually download the configuration files link:https://github.com/kubesphere/ks-installer/releases/download/v3.5.0/kubesphere-installer.yaml[kubesphere-installer.yaml] and link:https://github.com/kubesphere/ks-installer/releases/download/v3.5.0/cluster-configuration.yaml[cluster-configuration.yaml] from a device with internet access, and then transfer them to any node in the Kubernetes cluster.

...
. On the **Advanced Settings** tab, specify nodes for the pods managed by the workload and set the workload's metadata.

. On the **Cluster Difference Settings** tab, configure differentiated settings for the workload across different clusters based on containers, ports, and environment variables, then click **Create**. After the workload is created, it will be displayed in the workload list.
...
. On the **Resource Status** tab of the application details page, view the services and workloads that make up the application.
...
// |Whether application governance is enabled for the application. After application governance is enabled, you can use the traffic monitoring, grayscale release, and tracing features provided by KubeSphere.

// |Virtual IP Address
// |The virtual IP address for accessing the service from within the cluster. This is displayed only when the service's internal access type is **VirtualIP**.
|===
--
...
// |The current modification records of the workload.
|===
--
...
**Federated Cluster Application Management**: An extension designed to simplify the management of applications across multiple federated Kubernetes clusters. Users can easily deploy, update, and manage applications in multiple federated clusters.
...
**Gatekeeper**: Gatekeeper is an admission controller for Kubernetes that allows flexible policy configuration. It uses link:https://www.openpolicyagent.org/[Open Policy Agent (OPA)] to validate requests for creating and updating resources on a Kubernetes cluster.
...
// Compared to KubeSphere, there are 12 additional extensions, namely Auditing, Events Alerting, Ingress Tool, DMP, Spring Cloud, OAuth2-Proxy, NVIDIA GPU Operator, Certificate Management, Alloy, plus the 4 newly added extensions.
...
description: "介绍如何配置“WizTelemetry 日志”扩展组件。"
weight: 01
---

WizTelemetry 日志支持通过配置扩展组件实现收集指定日志、按项目创建日志索引等功能。
...
|
更多配置信息，请参阅扩展中心 “WizTelemetry 日志”扩展组件的详情页说明。
|===

== 收集指定日志

WizTelemetry 日志支持设置过滤条件，以收集包含或排除指定 namespace 的日志，可减少日志数量。

在 WizTelemetry 日志的扩展组件配置中，编辑如下参数：
...
WizTelemetry 日志支持为每个项目（namespace）创建一个索引，既可确定某个 namespace 的日志占用空间，方便整改业务，又便于进行日志分析。

=== 前提条件

版本要求：WizTelemetry 日志 >= 1.2.0，WizTelemetry 平台服务 >= 1.1.0。

=== 操作步骤

. 修改 WizTelemetry 日志扩展组件配置。无需修改 prefix 中的参数。
+
[,yaml]
----
vector-logging:
...
. 修改 WizTelemetry 平台服务扩展组件配置。无需修改 indexPrefix 中的参数。
+
[,yaml]
----
whizard-telemetry:

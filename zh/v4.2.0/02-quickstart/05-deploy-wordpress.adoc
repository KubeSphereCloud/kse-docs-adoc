...
本节以安装 WordPress 为例，演示如何在 KubeSphere Web 控制台部署应用程序，并在集群外进行访问。有关应用资源管理的更多信息，请参阅link:../../08-workspace-management/10-application-workloads[应用负载]。
endif::[]

ifeval::["{file_output_type}" == "pdf"]
本节以安装 WordPress 为例，演示如何在 KubeSphere Web 控制台部署应用程序，并在集群外进行访问。有关应用资源管理的更多信息，请参阅《KubeSphere 企业空间和项目管理指南》的“应用负载”章节。
endif::[]
...
. 在左侧导航栏，选择**应用负载** > **自制应用**，在页面右侧点击**创建**。

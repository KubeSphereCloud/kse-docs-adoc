...
KubeSphere 平台的存储类的默认回收机制为 **Delete**，即默认情况下持久卷声明删除后系统会自动删除持久卷。如果您创建了回收机制为 **Retain** 的持久卷或存储类，可能会存在持久卷声明被删除而持久卷未被删除的情况。有关更多信息，请参阅 link:https://kubernetes.io/zh/docs/concepts/storage/storage-classes/[Kubernetes 官方文档]。
